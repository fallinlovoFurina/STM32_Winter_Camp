...
void move_back_cm(float cm)
{
    ...
    // 通过PID计算实际PWM值（TT1=80, TT2=70）
    float left_pwm = PID_Calc(&PID_MotorLeft, 80, TIM_GetCapture1(MOTOR_TIM));
    float right_pwm = PID_Calc(&PID_MotorRight, 70, TIM_GetCapture2(MOTOR_TIM));

    // PWM限幅（0-99）
    left_pwm = (left_pwm > 99) ? 99 : (left_pwm < 0) ? 0 : left_pwm;
    right_pwm = (right_pwm > 99) ? 99 : (right_pwm < 0) ? 0 : right_pwm;
    ...
    while (1)
    {
        ...
        else if ((red1 == IR_NO_OBSTACLE) && (red2 == IR_NO_OBSTACLE) && (red3 || red5))
        {
            ...
            if (red3 == IR_HAVE_OBSTACLE && red5 == IR_HAVE_OBSTACLE)
            {
                ...
                left_pwm = PID_Calc(&PID_MotorLeft, 90, TIM_GetCapture1(MOTOR_TIM));
                right_pwm = PID_Calc(&PID_MotorRight, 70, TIM_GetCapture2(MOTOR_TIM));
            }
            else if (red3 == IR_HAVE_OBSTACLE)
            {
                // RED3单触：右轮加速、左轮正常（微调回正）
                left_pwm = PID_Calc(&PID_MotorLeft, 80, TIM_GetCapture1(MOTOR_TIM));
                right_pwm = PID_Calc(&PID_MotorRight, 80, TIM_GetCapture2(MOTOR_TIM));
            }
        ...
        else if ((red1 == IR_NO_OBSTACLE) && (red2 == IR_NO_OBSTACLE) && (red4 || red6))
        {
            ...
            if (red4 == IR_HAVE_OBSTACLE && red6 == IR_HAVE_OBSTACLE)
            {
                ...
                left_pwm = PID_Calc(&PID_MotorLeft, 80, TIM_GetCapture1(MOTOR_TIM));
                right_pwm = PID_Calc(&PID_MotorRight, 90, TIM_GetCapture2(MOTOR_TIM));
            }
        ...
        {
            // 低速直行（TT1=40, TT2=30）
            float left_pwm = PID_Calc(&PID_MotorLeft, 40, TIM_GetCapture1(MOTOR_TIM));
            float right_pwm = PID_Calc(&PID_MotorRight, 30, TIM_GetCapture2(MOTOR_TIM));
        ...
        {
            Motor_Forward(); // 正常直行（TT1=80, TT2=70）
        }
    }
}
